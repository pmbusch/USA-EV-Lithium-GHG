...
df <- df %>% 
  ...
  dplyr::select(gea,t,battery_MWh,biomass_MWh,canada_MWh,coal_MWh,
                csp_MWh,distpv_MWh,gas.cc_MWh,gas.ct_MWh,geothermal_MWh,
                hydro_MWh,nuclear_MWh,o.g.s_MWh,phs_MWh,upv_MWh,
...
df %>% 
  ...
  group_by(type) %>% 
  reframe(MWh=sum(MWh)) %>% ungroup() %>% 
  arrange(desc(MWh)) %>% mutate(share=MWh/sum(MWh))
...
ggsave("Figures/Electricity/Cambium_MixEnergy.png", ggplot2::last_plot(),units="cm",
...
ggsave("Figures/Electricity/Cambium_Mix.png", ggplot2::last_plot(),units="cm",
       dpi=600,width=18.5,height=9.7)
