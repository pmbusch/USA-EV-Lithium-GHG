...
egrid <- egrid %>%
  mutate(
    fuel = case_when(
      ...
      key == "SRGENAOF" ~ "Petroleum"
    )
  ) # other fossil, assume oil
...
egrid_ak <- egrid %>%
  filter(str_detect(SRNAME, "ASCC")) %>%
  filter(value > 0) %>%
  group_by(SRNAME) %>%
  mutate(share = value / sum(value)) %>%
  ungroup() %>%
  mutate(dummy = 1) %>%
  left_join(tibble(period = 2022:2050, dummy = 1)) %>%
  mutate(dummy = NULL)
...
ggsave(
  "Figures/Electricity/AK_HI_Mix.png",
  ggplot2::last_plot(),
  units = "cm",
  dpi = 600,
  width = 8.7 * 2,
