...
df_all <- ecoinvent %>% rename(Flow = Flows) %>% left_join(end, by = "Flow")
names(df_all)
...
energy <- df_all %>%
  ...
  ) %>% # all others do not require extraction
  filter(!near(MJ, 0)) %>%
  mutate(MJ = Amount * MJ) %>%
  mutate(
...
data_fig <- df %>%
  mutate(
    Name = str_remove(Name, "electricity production, ") %>%
...
data_fig <- df_all %>%
  ...
  reframe(MJ = sum(MJ), MJ_nonRenewable = sum(MJ_nonRenewable)) %>%
  ungroup()

# average by region
data_fig <- data_fig %>%
  group_by(Name, Flow) %>%
  reframe(MJ = mean(MJ), MJ_nonRenewable = mean(MJ_nonRenewable)) %>%
  ungroup()
...
ggsave(
  "Figures/Electricity/ecoinventMaterials.png",
  ggplot2::last_plot(),
  units = "cm",
  dpi = 600,
  width = 8.7 * 3,
