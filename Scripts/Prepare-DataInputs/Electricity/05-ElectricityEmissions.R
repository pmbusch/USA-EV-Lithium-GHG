...
# Match fuels
impacts <- impacts %>% filter(str_detect(Name,"electricity production|waste incineration")) %>% 
  mutate(Name=str_remove(Name,"electricity production, "))
unique(impacts$Name)
unique(df$fuel)
join_fuel <- read.csv("Inputs/join_fuels.csv")
impacts <- impacts %>% rename(ecoinvent_fuel=Name) %>% 
  left_join(join_fuel) %>% 
...
# national avg
impacts_nat <- impacts %>% group_by(fuel) %>% 
  reframe(across(-c(sheet,ecoinvent_fuel,Region),~ mean(.x))) %>% ungroup() 
...
df_impacts <- df_impacts %>% 
  ...
  mutate(kgCO2eq=if_else(is.na(kgCO2eq),nat_kgCO2eq,kgCO2eq),
...
electricity <- df_impacts %>% 
  mutate(across(-c(State,NAMELSAD,STATEFP,COUNTYFP,NAME,pop,Region_EMM,
                   Region_Electricity_EIA,
                   Region_ecoinvent_detail,Region_ecoinvent,Region_Transport,
                   scenario,period,fuel,share,sheet),
                ~ .x * share)) %>%  
  group_by(scenario,State,STATEFP,COUNTYFP,NAME,period) %>% 
  reframe(pop=mean(pop),
          # share=sum(share), # to debug
          across(-c(NAMELSAD,pop,Region_EMM,sheet,share,Region_Electricity_EIA,fuel,
                    Region_ecoinvent_detail,Region_ecoinvent,Region_Transport,),
                 ~ sum(.x))) %>%  ungroup()
...
electricity <- electricity %>% 
  left_join(td_loss) %>% 
  mutate(across(-c(scenario,State,STATEFP,COUNTYFP,
                   NAME,period,pop),~ .x /(1-losses) )) %>% 
  mutate(losses=NULL)
...
electricity_state <- electricity %>% 
  ...
  reframe(kgCO2eq=weighted.mean(kgCO2eq,pop)) %>% ungroup()
...
for (i in unique(df$scenario)){
  ...
  new_data <- geo_shifted %>% mutate(kgCO2eq=NULL) %>% 
...
impacts_material <- impacts_material %>% filter(str_detect(Name,"electricity production|waste incineration")) %>% 
  mutate(Name=str_remove(Name,"electricity production, "))
...
impacts_nat_material <- impacts_material %>% 
  group_by(fuel) %>% 
  reframe(across(starts_with("kg"), ~ mean(.x))) %>% ungroup()
...
df_impacts_mat <- df_impacts_mat %>% 
  ...
  mutate(kgMat=if_else(is.na(kgMat),nat_kgMat,kgMat),
  ...
  dplyr::select(-nat_kgMat,-nat_kgMetal,-nat_kgAluminium,-nat_kgCobalt,
                -nat_kgCopper,-nat_kgLithium,-nat_kgNickel,-nat_kgREE,
                -nat_kgZinc,-nat_kgBarium,-nat_kgCalcium,-nat_kgChromium,
                -nat_kgIron,-nat_kgMagnesium,-nat_kgManganese,-nat_kgPhosphorus,
                -nat_kgSilicon,-nat_kgSulphur)
...
electricity_mat <- df_impacts_mat %>% 
  ...
  reframe(pop=mean(pop),
          # share=sum(share), # to debug
          across(starts_with("kg"), ~ sum(.x))) %>% ungroup()
...
electricity_mat <- electricity_mat %>% 
  left_join(td_loss) %>% 
  mutate(across(starts_with("kg"), ~ .x/(1-losses)),
         losses=NULL)
