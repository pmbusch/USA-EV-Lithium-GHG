...
ev <- ev %>% 
  mutate(sales_2023=Reg_2023-Reg_2022,
         ...
         sales_2016=Reg_2016, # use 2016 proportions, sales were low
         sales_2015=Reg_2016)      
...
ev_share <- ev %>%  
  mutate(pop=pop/sum(pop),
         ev=sales_2023/sum(sales_2023)) %>% ungroup() %>% 
...
ggsave("Figures/Fleet/EV_reg_share.png", ggplot2::last_plot(),
       units="cm",dpi=600,width=8.7*1.5,height=14)
...
ev_share_historical <- ev %>% 
  ...
  group_by(period) %>% 
  mutate(ev_share=unit/sum(unit)) %>% ungroup() %>% dplyr::select(period,State,ev_share)
...
# Figure
labels <- df_share %>% left_join(st_code) %>%
  filter(period==2050) %>% 
...
ggsave("Figures/Fleet/Sales_EV_dissagregated.png", ggplot2::last_plot(),
       units="cm",dpi=600,width=8.7*2,height=10)
...
sales <- read.csv("Inputs/region_outflows_LIB.csv",
...
sales <- df_share %>% left_join(sales) %>% 
