...
lib_upstream <- lib_upstream %>%  
  ...
  reframe(across(-c(LIB_Chem), ~ sum(.x) )) %>% ungroup()
...
intensity %>% 
  ...
  pivot_longer(-c(Mineral,chemistry), 
...
ggsave("Figures/Fleet/MineralIntensity.png", ggplot2::last_plot(),
...
batpac <- batpac %>% group_by(Scenario_Capacity,vehSize,Mineral) %>% 
  reframe(kg_veh=sum(kg_veh)) %>% ungroup() # in kg

batpac %>% pivot_wider(names_from = Mineral, values_from = kg_veh)
lib_upstream %>% dplyr::select(Scenario_Capacity,vehSize,LIB_kgLithium,
                               LIB_kgCobalt,LIB_kgCopper,LIB_kgNickel)
