...
lib_recyc_upstream <- upstream %>% 
  filter(str_detect(Name,"treatment of used"))
...
names(lib_recyc_upstream)[-(36:38)] <- paste0("LIBRecyc_",names(lib_recyc_upstream)[-(36:38)])
...
lib_recyc_upstream2 <- lib_recyc_upstream %>% 
  ...
  mutate(value=if_else(
    str_detect(key,"Aluminum|Cobalt|Copper|Lithium|Nickel|REE|Zinc|Barium|Calcium|Chromium|Iron|Magnesium|Manganese|Phosphorus|Silicon|Sulphur"),
    value-subst,value),
    subst=NULL) %>% 
