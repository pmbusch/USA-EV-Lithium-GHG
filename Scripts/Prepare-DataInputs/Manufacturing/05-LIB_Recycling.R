...
lib_recyc_upstream <- upstream %>% filter(str_detect(Name, "treatment of used Li"))
...
bat_agg <- bat %>%
  ...
  group_by(Scenario_Capacity, vehSize) %>%
  reframe(kwh_veh = sum(kwh_veh), Wh_kg = weighted.mean(Wh_kg, share)) %>%
  ungroup()
...
# Maximum recovery rates - assumed based on EU and China legistlation
recovery_assumptions <- tibble(
  Mineral = c("Nickel", "Cobalt", "Lithium", "Copper", "Aluminium", "REE", "Lead", "Manganese"),
  recov_rate = c(0.9, 0.9, 0.8, 0.95, 0.95, 0.95, 0.9, 0.95)
)
...
tot <- recovery %>% group_by(Scenario_Capacity, vehSize) %>% reframe(subst = sum(subst)) %>% ungroup()
recovery <- rbind(
  recovery,
