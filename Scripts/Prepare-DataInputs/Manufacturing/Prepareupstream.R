...
LIB_BOM <- read_excel(sprintf(url_file, "GREET2024_Data.xlsx"), sheet = "LIB_BoM", col_names = FALSE)
...
for (i in seq(2, 190, by = 3)) {
  ...
  battery_row <- tibble(Type = type, Range = range, LIB_Chem = chem, !!!param_values)

  battery_data_list[[length(battery_data_list) + 1]] <- battery_row
}

LIB_BOM_tidy <- bind_rows(battery_data_list)
LIB_BOM_tidy <- LIB_BOM_tidy %>% mutate(Type = if_else(Type == "Car", "CAR", Type))
...
raw <- read_excel(sprintf(url_file, "GREET2024_Data.xlsx"), sheet = "LIB_Specific Energy", col_names = FALSE)
...
for (i in seq(1, nrow(raw), by = 4)) {
  ...
  for (j in seq_along(col_blocks)) {
    ...
    df <- tibble(Type = model, Range = range, LIB_Chem = chemistries, Spec_Energy_WhPerKg = values) %>%
...
LIB_upstream <- LIB_BOM_tidy %>%
  left_join(
    LIB_SpecEnergy %>% select(Type, Range, LIB_Chem, Spec_Energy_WhPerKg),
...
upstream_libmaterial <- LIB_upstream_GHG %>%
  filter(Type %in% c("CAR", "PUT"), Range == "300mile", LIB_Chem != "LMO") %>%
...
LIB_Assembly <- read_excel(sprintf(url_file, "GREET2024_Data.xlsx"), sheet = "LIB_Assembly", col_names = FALSE)
...
LIB_Assembly <- LIB_Assembly %>% mutate(GWP_factor = GWP_factors[emission], GWP_component = g_per_kWh * GWP_factor)
write.csv(LIB_Assembly, "Inputs/LIB_Assembly.csv", row.names = FALSE)


# Gasoline kg CO2e/gal---------

Gasoline <- read_excel(sprintf(url_file, "GREET2024_Data.xlsx"), sheet = "Gasoline", col_names = FALSE)
...
Gas_upstream <- Gas_upstream %>% mutate(g_per_gal = g_per_mmbtu * gasoline_LHV / 1e6)

GWP_factors <- c("CO2 (w/ C in VOC & CO)" = 1, "CH4" = 29.8, "N2O" = 273)

Gas_upstream <- Gas_upstream %>%
  mutate(g_per_gal = as.numeric(g_per_gal), GWP_factor = GWP_factors[emission], GWP_component = g_per_gal * GWP_factor)
write_csv(Gas_upstream, "Inputs/Gas_upstream.csv")


# Vehicle Production(Exclude EV Battery)---------

Vehi_Prod <- read_excel(sprintf(url_file, "GREET2024_Data.xlsx"), sheet = "Vehi_Prod", col_names = FALSE)
...
Prod_Veh <- bind_rows(Prod_CAR_ICE, Prod_CAR_BEV, Prod_SUV_ICE, Prod_SUV_BEV, Prod_PUT_ICE, Prod_PUT_BEV) %>%
  mutate(GWP_factor = GWP_factors[Emissions], GWP_component = g_per_vehlife * GWP_factor)
write_csv(Prod_Veh, "Inputs/Prod_Veh.csv")


# LIB Recycling ---------

LIB_Rec <- read_excel(sprintf(url_file, "GREET2024_Data.xlsx"), sheet = "LIB_Recycling", col_names = FALSE)
...
emission_values <- LIB_Rec[10:20, 2:8] %>% mutate_all(as.numeric)
...
Li_Extraction <- read_excel(sprintf(url_file, "GREET2024_Data.xlsx"), sheet = "Lithium_Extraction", col_names = FALSE)
...
Li_exact <- Li_Extraction[13:25, ] %>%
  ...
  mutate(gwp = GWP_factors[name], co2e_g = ifelse(!is.na(gwp), as.numeric(value) * gwp, NA))
