...
# Census Pop data -------
census <- readxl::read_excel(paste0(url_file,"/Census/co-est2023-pop.xlsx"),
                             sheet="CO-EST2023-POP",range="A5:F3149")
...
# shp_emm <- project(shp_emm, crs(shp))
...
intersections <- intersections %>% group_by(State,NAMELSAD) %>% 
  mutate(n=n()) %>% ungroup()
...
intersections %>% filter(propArea>0.5) %>% 
...
result <-intersections %>% 
  group_by(State,NAMELSAD) %>% 
  mutate(maxOverlap=max(propArea)) %>% ungroup() %>% 
...
# check failed joins - Alaska and Hawai are not part of EIA data 
as.data.frame(shp) %>% filter(is.na(eGrid_Reg)) %>% 
  filter(!State %in% c("Alaska","Hawaii"))
# both in Massachusetts, due to different map resolution, check region
as.data.frame(shp) %>% filter(State=="Massachusetts") %>% 
  group_by(eGrid_Reg,Region_Electricity) %>% tally() 
# simply use this region, as both counties are on the coast
aux <- as.data.frame(shp) %>%
  mutate(eGrid_Reg = if_else(State=="Massachusetts"&is.na(eGrid_Reg),"ISNE",eGrid_Reg),
         Region_Electricity = if_else(State=="Massachusetts"&is.na(Region_Electricity),"Northeast Power Coordinating Council / New England",Region_Electricity))
values(shp) <- aux
rm(aux)


## Join to ecoinvent or egrid -----

# same projection
crs(shp)
crs(shp_ecoinvent)

terra::plot(shp_ecoinvent, border = "blue")
terra::plot(shp, border = "darkred", add = TRUE)
...
as.data.frame(shp) %>% filter(State %in% c("Massachusetts","Hawaii","Washington")) %>% 
  group_by(State,fullName,Region) %>% tally() 
# trivial regions to assing

# simply use this region, as both counties are on the coast
aux <- as.data.frame(shp) %>%
  mutate(fullName = if_else(State=="Massachusetts"&is.na(fullName),"NPCC New England",fullName),
...
df <- df %>% dplyr::select(State,NAMELSAD,STATEFP,COUNTYFP,NAME,
                           pop,Region_Transport,eGrid_Reg,Region_Electricity,
                           fullName,Region) %>% 
