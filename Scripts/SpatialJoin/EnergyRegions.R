...
dict_ecoinvent <- tibble(fullName=unique(shp_ecoinvent$fullName)) %>% 
  mutate(Region=paste0("US-",str_extract(fullName,
                                         "WECC|NPCC|ASCC|SERC|RFC|HICC|MRO|ERCOT|FRCC|SPP")) %>% 
           str_replace("ERCOT","TRE") %>% # Texas case
           str_replace("FRCC","SERC") %>% # Florida does not have ecoinvent - use SERC (old maps)
           str_replace("SPP","MRO")) %>%  # Region with no ecoinvent, use MRO (old maps)
  filter(Region!="US-NA")
dict_ecoinvent
...
shp_ecoinvent$NAME <- shp_ecoinvent$dataYear <- shp_ecoinvent$emissionFactor <- shp_ecoinvent$type <- NULL
shp_ecoinvent$name <- shp_ecoinvent$CENSUSAREA <- shp_ecoinvent$gridLoss <- shp_ecoinvent$LSAD <- NULL
